...
const LoginInPage = () => {
  ...
  const handleSubmit = (e) => {
    ...
    setLoggedIn(true); // mark as successful
  };
  ...
  return (
    <div className="flex min-h-screen font-poppins bg-mint/10">
      ...
      <div className="flex-1 flex flex-col justify-center px-16 lg:px-28 max-w-4xl bg-white relative ">
        <div>
          <h1 className="text-3xl font-semibold text-darkGreen">
            {loggedIn ? "Successfully Logged In!" : "Welcome Back"}
          </h1>
        </div>

        {/* Form only shows if not logged in */}
        {!loggedIn && (
          <form onSubmit={handleSubmit} className="mt-10 flex flex-col gap-5 max-w-sm">
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border-2 transition-all duration-300 focus:outline-none focus:ring-4 ${
                  emailActive ? "border-forest ring-forest/20" : "border-gray-200 hover:border-gray-400"
                }`}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-2.5 rounded-xl border-2 transition-all duration-300 focus:outline-none focus:ring-4 ${
                  passwordActive ? "border-forest ring-forest/20" : "border-gray-200 hover:border-gray-400"
                }`}
              />
            </div>
            ...
            <div className="flex items-center justify-center gap-2 mt-6 text-gray-500">
              ...
              <Link to="/signup" className="text-sm text-pine font-medium hover:underline">
                Sign up
              </Link>
            </div>
          </form>
      ...
      {/* RIGHT — VISUAL PANEL remains unchanged */}
      ...
      <div className="absolute hidden md:block border">
      ...
      <style>{`
        ...
        .animate-pulseWave { animation: pulseWave 3s ease-out infinite; }

        @keyframes fadeInSlow {
          from { opacity: 0; transform: translateY(10px); }
          to { opacity: 1; transform: translateY(0); }
        }
        .animate-fadeInSlow { animation: fadeInSlow 1.6s ease-out forwards; }
      `}</style>
    </div>
  );
};
